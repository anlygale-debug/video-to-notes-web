...
> Extracted from `https://stripe.com` on July 2, 2026
> 3421 elements analyzed
...
|------|-----|-----|-----|-------------|
| Primary | `#533afd` | rgb(83, 58, 253) | hsl(248, 98%, 61%) | 909 |
...
| Hex | HSL | Usage Count |
|-----|-----|-------------|
| `#000000` | hsl(0, 0%, 0%) | 3407 |
| `#50617a` | hsl(216, 21%, 40%) | 514 |
| `#ffffff` | hsl(0, 0%, 100%) | 340 |
| `#64748d` | hsl(217, 17%, 47%) | 272 |
...
Used on large-area elements: `#ffffff`, `#e5edf5`, `#f8fafd`, `#5d0252`, `#fffafd`, `#0d1738`, `#122054`, `#510002`
...
```css
background-image: conic-gradient(from 346deg, rgb(255, 215, 239) 0%, rgb(255, 215, 239) 25%, rgb(165, 29, 133) 50%, rgb(255, 215, 239) 75%, rgb(255, 215, 239) 100%);
```

```css
background-image: conic-gradient(from 360deg, rgb(255, 215, 239) 0%, rgb(255, 215, 239) 25%, rgb(165, 29, 133) 50%, rgb(255, 215, 239) 75%, rgb(255, 215, 239) 100%);
```
...
| Hex | Contexts | Count |
|-----|----------|-------|
| `#000000` | text, border, background | 3407 |
| `#533afd` | text, border, background | 909 |
| `#061b31` | text, border, background | 903 |
| `#50617a` | text, border | 514 |
| `#ffffff` | background, text, border | 340 |
| `#64748d` | text, border | 272 |
...
- **sohne-var** — used for all (3347 elements)
...
`400` (2690x), `300` (654x), `500` (76x), `700` (1x)
...
**102 grid containers** and **532 flex containers** detected.
...
|----------|-----------|-------------|-------------|-----------|-------------|
| mobile (375px) | 16px | Yes | 12 | Yes | 20165px |
| tablet (768px) | 16px | Yes | 12 | Yes | 14921px |
| desktop (1280px) | 16px | Yes | 12 | No | 14495px |
| wide (1920px) | 16px | Yes | 12 | No | 14736px |

### Breakpoint Changes

**375px → 768px** (mobile → tablet):
- H1 size: `34px` → `40px`
- Page height: `20165px` → `14921px`

**768px → 1280px** (tablet → desktop):
- H1 size: `40px` → `48px`
- Hamburger menu: `shown` → `hidden`
- Page height: `14921px` → `14495px`

**1280px → 1920px** (desktop → wide):
- Page height: `14495px` → `14736px`
...
```css
/* Focus */
outline: rgb(6, 27, 49) none 3px → rgba(84, 82, 251, 0.75) solid 3px;
```

**"Solutions"**
```css
/* Hover */
color: rgb(40, 59, 82) → rgb(26, 46, 69);
border-color: rgb(40, 59, 82) → rgb(26, 46, 69);
outline: rgb(40, 59, 82) none 3px → rgb(26, 46, 69) none 3px;
```
```css
/* Focus */
color: rgb(40, 59, 82) → rgb(6, 27, 49);
border-color: rgb(40, 59, 82) → rgb(6, 27, 49);
outline: rgb(40, 59, 82) none 3px → rgba(84, 82, 251, 0.75) solid 3px;
```

**"Developers"**
```css
/* Hover */
color: rgb(100, 116, 141) → rgb(77, 94, 118);
border-color: rgb(100, 116, 141) → rgb(77, 94, 118);
outline: rgb(100, 116, 141) none 3px → rgb(77, 94, 118) none 3px;
```
```css
/* Focus */
color: rgb(100, 116, 141) → rgb(17, 38, 60);
border-color: rgb(100, 116, 141) → rgb(17, 38, 60);
outline: rgb(100, 116, 141) none 3px → rgba(84, 82, 251, 0.75) solid 3px;
```
...
**Overall Score: 77%** — 17 passing, 5 failing color pairs
...
- 6458 duplicate CSS declarations

## Gradients

**22 unique gradients** detected.
...
**136 unique SVG icons** detected. Dominant style: **filled**.
...
| general | 17 | objectFit: fill, borderRadius: 0px, shape: square |
| gallery | 14 | objectFit: fill, borderRadius: 0px, shape: square |
| hero | 3 | objectFit: cover, borderRadius: 0px, shape: square |

**Aspect ratios:** 4.18:1 (36x), 21:9 (11x), 3:4 (10x), 1:1 (9x), 4:3 (6x), 9:16 (4x), 3:2 (2x), 2:3 (2x)
...
- **custom** (70 uses) — `cubic-bezier(0.45, 0.05, 0.55, 0.95)`, `cubic-bezier(0.4, 0, 0.2, 1)`, `cubic-bezier(0.6, 0, 0.2, 0.5)`
- **ease-out** (560 uses) — `cubic-bezier(0.25, 1, 0.5, 1)`, `cubic-bezier(0.165, 0.84, 0.44, 1)`, `cubic-bezier(0.16, 1, 0.3, 1)`
...
|---|---|---|
| default | 206 | Accept and optimise payments globally –  |
...
Reading order (top→bottom): cta → nav → pricing-table → hero → pricing-table → content → stats → pricing-table → testimonials → nav → pricing-table → nav → gallery → pricing-table → nav → pricing-table → pricing-table → pricing-table → gallery → cta → nav → nav → footer
...
| 14 | gallery | — | 0.7 |
| 15 | pricing-table | Make your SaaS platform a complete financial operating system | 0.9 |
| 16 | nav | Make your SaaS platform a complete financial operating system | 0.4 |
| 17 | pricing-table | Reliable, extensible infrastructure for every stack. | 0.9 |
...
**Label:** `flat-illustration` (confidence 0.233)
**Counts:** total 86, svg 44, icon 1, screenshot-like 0, photo-like 2
...
|-----------|-----|--------|
| pricing | https://stripe.com/en-hk/pricing | ok |
